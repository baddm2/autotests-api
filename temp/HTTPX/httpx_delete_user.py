...
# 3. Удаляем юзера по его идентификатору

userId = create_user_response_data["user"]["id"]
token = {"Authorization": f"Bearer {login_response_data["token"]["accessToken"]}"}

delete_user_response = httpx.delete(f"http://localhost:8000/api/v1/users/{userId}", headers=token)
delete_user_response_data = delete_user_response.json()
print(f"Пользователь удален успешно: {delete_user_response.status_code}")

# 4. Получаем инфу о юзере по его идентификатору
